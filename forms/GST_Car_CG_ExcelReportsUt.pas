unit GST_Car_CG_ExcelReportsUt;
...
procedure GST_Car_CG_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin, x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
procedure PrintState (scExcelExport: TScExcelExport; GpSds: TSQLDataSet; var x_row: integer; x_StartRow: integer);
procedure PrintSector (scExcelExport: TScExcelExport; GpSds: TSQLDataSet; var x_row: integer);
...
procedure GST_Car_CG_LineWise_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin, x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
...
procedure GST_Car_CG_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin,
                                x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
var
  GpSds: TSQLDataSet;
  x_row, x_StartRow, x_EndRow, x_type, x_count : integer;
  x_PrevStates_id, x_PrevAgents_id, x_States_id, x_Agents_id, x_PrevCarHireGroups_id, x_CarHireGroups_id: integer;
begin
  ...
  if (x_ReportType = 1) or (x_ReportType = 2) then
    scExcelExport.Filename := ExtractFilePath(Application.ExeName) + 'Excel\CarHire_CG_PriceList.xlsx'
  else
    scExcelExport.Filename := ExtractFilePath(Application.ExeName) + 'Excel\CarHire_CG_PriceList_1_10.xlsx';
  ...
  GpSds.Close;
  GpSds.CommandText := 'exec [p_CarHire_CityGroup_PriceList_GST] ' +
        QuotedStr(FormatDateTime('mm/dd/yyyy',x_TravelDate)) + ', ' +
  ...
  if (x_ReportType = 1) then
   if (not GpSds.Eof) and (GpSds['TourOperatorGstPerc'] <> null) then
     scExcelExport.ExcelWorkSheet.Range['W3','W3'].Value := 'Tour Gst @ ' + FormatFloat('#,##0.0',GpSds['TourOperatorGstPerc']) + '%';

  x_StartRow := 8;
  x_row := x_StartRow;

  x_PrevStates_id := -1;
  x_PrevCarHireGroups_id := -1;
  x_PrevAgents_id := -1;
  x_count := 0;
  ...
  while (not GpSds.EOF) do
    begin
      ...
      x_CarHireGroups_id := GpSds['CarHireGroups_id'];
      if (x_CarHireGroups_id <> x_PrevCarHireGroups_id) then
        PrintSector (scExcelExport, GpSds, x_row);

      x_Agents_id := GpSds['Agents_id'];
      if (x_count = 0) then
        x_type := 2
      else if (x_CarHireGroups_id <> x_PrevCarHireGroups_id) then
        x_type := 3
      ...
      x_PrevCarHireGroups_id := x_CarHireGroups_id;
      x_PrevAgents_id := x_Agents_id;

      if (x_count = 0) and (GpSds['MarginPerc'] <> null) and (x_ReportType = 1) then
        scExcelExport.ExcelWorkSheet.Range['T6','T6'].Value := '@ ' + FormatFloat('#,##0',GpSds['MarginPerc']) + '%';
  ...
  scExcelExport.ExcelWorkSheet.Range['I'+IntToStr(x_StartRow),'W'+IntToStr(x_EndRow)].NumberFormat := '#,##0';
  SetColumnColours (scExcelExport, x_StartRow, x_EndRow, x_ReportType);

  if (x_margin = '0') then
    scExcelExport.ExcelWorkSheet.Range['T1','W1'].EntireColumn.Hidden := true;
...
begin
  ...
  if GpSds['CarHireGroup'] <> null then
    scExcelExport.ExcelWorkSheet.Range['B'+IntToStr(x_row),'B'+IntToStr(x_row)].Value := GpSds['CarHireGroup'];
...
begin
  ...
  x_cell := NextColumn(x_cell);
  if (GpSds['ParkCost'] <> null) and (GpSds['ParkCost'] <> 0) then
    scExcelExport.ExcelWorkSheet.Range[x_cell+IntToStr(x_row),x_cell+IntToStr(x_row)].Value := GpSds['ParkCost'];
...
begin
  ...
  Gp2Sds.CommandText := 'SELECT v.Vehicle, t.ToPax, t.FinalQuoteCurr ' +
   ...
   'AND t.CarHireGroups_id = ' + IntToStr(GpSds['CarHireGroups_id']) + ' ' +
...
procedure GST_Car_CG_LineWise_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin,
                                         x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
var
  GpSds: TSQLDataSet;
  x_row, x_StartRow, x_EndRow, x_type, x_count : integer;
...
begin
  ...
  if (x_ReportType = 4) then
    begin
      scExcelExport.Filename := ExtractFilePath(Application.ExeName) + 'Excel\CarHire_CG_PriceList_1_10_SingleLine.xlsx'
    end;
