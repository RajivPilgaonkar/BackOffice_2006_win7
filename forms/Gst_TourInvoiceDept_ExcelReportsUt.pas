unit Gst_TourInvoiceDept_ExcelReportsUt;
...
procedure GST_TourInvoiceDept_Excel(scExcelExport: TScExcelExport; x_FromDate, x_ToDate: TDateTime; x_PrincipalAgents_id: integer; x_FileName: string; x_InvoiceFlag: integer);
procedure GST_TourInvoice_CommonSettings(scExcelExport: TScExcelExport; x_InvoiceFlag: integer);
procedure GstTourInvoiceDept_Header (scExcelExport: TScExcelExport; var x_row: integer; x_Invoices_id, x_InvoiceFlag: integer; x_PlaceOfSupply, x_InvoiceSuffix: string);
procedure GstManualTourInvoice_Header (scExcelExport: TScExcelExport; var x_row: integer; x_Invoices_id, x_InvoiceFlag: integer);
procedure GstTourInvoiceDept_PaxNames (scExcelExport: TScExcelExport; var x_row: integer; x_Invoices_id: integer);
procedure GstTourInvoiceDept_Details (scExcelExport: TScExcelExport; var x_row: integer; x_Invoices_id, x_InvoiceFlag, x_Manual: integer; x_PlaceOfSupply: string);
...
var
  ...
  x_cell, x_PlaceOfSupply, x_InvoiceSuffix: string;
begin
  ...
  GpSds.CommandText := 'SELECT DISTINCT i.invoices_id, i.InvoiceNo, COALESCE(id1.PlaceOfSupplyLine,'''') AS PlaceOfSupplyLine ' +
        'FROM Invoices i ' +
        'LEFT JOIN InvoiceDetails id1 ON i.Invoices_id = id1.invoices_id ' +
        ...
        'AND id1.unitprice <> 0.0 ' +
        'ORDER BY InvoiceNo, COALESCE(id1.PlaceOfSupplyLine,'''') ';
  ...
  while (not GpSds.EOF) do
    begin
      ...
      if (GpSds['Invoices_id'] <> x_invoices_id) then
        x_InvoiceSuffix := 'A'
      else
        x_InvoiceSuffix := NextColumn(x_InvoiceSuffix);

      x_Invoices_id := GpSds['Invoices_id'];

      x_PlaceOfSupply := '';
      if (GpSds['PlaceOfSupplyLine'] <> null) then
        x_PlaceOfSupply := GpSds['PlaceOfSupplyLine'];

      GstTourInvoiceDept_Header (scExcelExport, x_row, x_Invoices_id, x_InvoiceFlag, x_PlaceOfSupply, x_InvoiceSuffix);
      GstTourInvoiceDept_PaxNames (scExcelExport, x_row, x_Invoices_id);
      GstTourInvoiceDept_Details (scExcelExport, x_row, x_Invoices_id, x_InvoiceFlag, 0, x_PlaceOfSupply);
...
var
  ...
  x_GstinRecipient, x_SupplyState, x_TaxPayableRcm: string;
begin
  ...
  while (not GpSds.Eof) do
    begin
      ...
      if GpSds['InvoiceNo'] <> null then
        begin
          scExcelExport.ExcelWorkSheet.Range['B'+IntToStr(x_row),'B'+IntToStr(x_row)].Value := IntTOStr(GpSds['InvoiceNo']) + x_InvoiceSuffix;
      ...
      if x_InvoiceFlag = 1 then
        ...
        begin
          ...
          scExcelExport.ExcelWorkSheet.Range['A'+IntToStr(x_row),'J'+IntToStr(x_row)].Borders[xlEdgeBottom].LineStyle := xlContinuous;
          scExcelExport.ExcelWorkSheet.Range['A'+IntToStr(x_row),'J'+IntToStr(x_row)].Borders[xlEdgeBottom].Weight := xlMedium;
  ...
  x_EndRow := x_row-1;
  scExcelExport.ExcelWorkSheet.Range['A'+IntToStr(x_StartRow),'J'+IntToStr(x_EndRow)].Font.Size := '10';
  scExcelExport.ExcelWorkSheet.Range['A'+IntToStr(x_StartRow),'J'+IntToStr(x_EndRow)].Font.Name := 'Book Antiqua';
...
begin
  ...
  if (x_Manual = 1) then
    x_option := 4;
  GpSds.CommandText := 'EXEC p_PrintTourGstInvDept ' + IntToStr(x_Invoices_id) + ',''' + x_PlaceOfSupply + ''',' + IntToStr(x_option);
...
begin
  ...
  Gp1Sds.CommandText := 'EXEC [p_PrintTourGstInvDeptSummary] ' +  IntToStr(x_PrincipalAgents_id) + ',' +
    IntToStr(x_month) + ',' + IntToStr(x_year) + ',' + IntToStr(x_companies_id) + ',' +
    IntToStr(x_divisions_id) + ',' + IntToStr(x_offices_id) + ',1';

  Gp1Sds.Open;

  Gp2Sds.CommandText := 'EXEC [p_PrintTourGstInvDeptSummary] ' +  IntToStr(x_PrincipalAgents_id) + ',' +
    IntToStr(x_month) + ',' + IntToStr(x_year) + ',' + IntToStr(x_companies_id) + ',' +
  ...
  if x_InvoiceFlag = 2 then
    begin
      ...
      scExcelExport.ExcelWorkSheet.Range['J'+IntToStr(x_row),'J'+IntToStr(x_row)].Value := 'Place Of Supply';
    end;

  scExcelExport.ExcelWorkSheet.Range['E'+IntToStr(x_row),'E'+IntToStr(x_row)].HorizontalAlignment := xlCenter;
  scExcelExport.ExcelWorkSheet.Range['H'+IntToStr(x_row),'I'+IntToStr(x_row)].HorizontalAlignment := xlRight;
  scExcelExport.ExcelWorkSheet.Range['J'+IntToStr(x_row),'J'+IntToStr(x_row)].HorizontalAlignment := xlLeft;
  ...
  while not Gp2Sds.EOF do
    begin
      ...
      x_InvoiceSuffix := '';
      if Gp2Sds['InvoiceSuffix'] <> null then
        x_InvoiceSuffix := Gp2Sds['InvoiceSuffix'];

      if Gp2Sds['InvoiceNo'] <> null then
        scExcelExport.ExcelWorkSheet.Range['B'+IntToStr(x_row),'B'+IntToStr(x_row)].Value := IntToStr(Gp2Sds['InvoiceNo']) + x_InvoiceSuffix;
      ...
      if x_InvoiceFlag = 2 then
        begin
          ...
          scExcelExport.ExcelWorkSheet.Range['I'+IntToStr(x_row),'I'+IntToStr(x_row)].NumberFormat := '#,##0.000';
          scExcelExport.ExcelWorkSheet.Range['J'+IntToStr(x_row),'J'+IntToStr(x_row)].Value := Gp2Sds['PlaceOfSupply'];
        end;
  ...
  scExcelExport.ExcelWorkSheet.Range['B'+IntToStr(x_row),'J'+IntToStr(x_row)].Font.FontStyle := 'Bold';
