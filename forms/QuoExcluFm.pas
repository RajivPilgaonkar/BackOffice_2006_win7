...
type
  TQuoExcluForm = class(TCustom_1MG_1DG_Form)
...
var
  QuoExcluForm: TQuoExcluForm;
  _QuoExcluForm_Level: integer;
  _QuoExcluForm_Quotations_id: integer;
...
procedure TQuoExcluForm.FormCreate(Sender: TObject);
begin
  inherited;
  ...
  _QuoExcluForm_Level := 4;

  inherited;

end;

procedure TQuoExcluForm.FormClose(Sender: TObject;
...
procedure TQuoExcluForm.FormDestroy(Sender: TObject);
begin
  inherited;
  QuoExcluForm := nil;
end;

procedure TQuoExcluForm.ActivateInCustom;
begin
  ...
  Custom_AdmLevel := _QuoExcluForm_Level;
...
procedure TQuoExcluForm.FormActivate(Sender: TObject);
begin
  inherited;
  ActivateInCustom;
  cxGrid1.SetFocus;
...
procedure TQuoExcluForm.MasterCdsBeforePost(DataSet: TDataSet);
begin
...
procedure TQuoExcluForm.DetailCdsBeforePost(DataSet: TDataSet);
begin
...
function TQuoExcluForm.IsMasterRecordDuplicate: Boolean;
var
...
function TQuoExcluForm.GetNextMasterSrNo: integer;
var
  GpSds : TSQLDataSet;
  x_Max_SrNo: integer;
begin
...
function TQuoExcluForm.GetNextDetailSrNo(QuoExclusions_id: integer): integer;
var
  GpSds : TSQLDataSet;
  x_Max_SrNo: integer;
begin
...
procedure TQuoExcluForm.DetailCdsAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TQuoExcluForm.MasterCdsAfterInsert(DataSet: TDataSet);
begin
  inherited;
  MasterCds['SrNo'] := GetNextMasterSrNo;
end;

procedure TQuoExcluForm.AgentExclusions1Click(Sender: TObject);
begin
  inherited;
  QuoExclusionDisplayForm_level := 4;
  QuoExclusionDisplayForm_QuoExclusions_id := MasterCds['QuoExclusions_id'];
  QuoExclusionDisplayForm_Quotations_id := _QuoExcluForm_Quotations_id;
