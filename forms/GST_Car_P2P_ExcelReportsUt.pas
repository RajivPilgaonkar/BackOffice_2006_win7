unit GST_Car_P2P_ExcelReportsUt;
...
procedure GST_Car_P2P_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin, x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
...
procedure GST_Car_P2P_LineWise_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin, x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
...
procedure GST_Car_P2P_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin,
...
begin
  ...
  if (x_ReportType = 1) or (x_ReportType = 2) then
    scExcelExport.Filename := ExtractFilePath(Application.ExeName) + 'Excel\CarHire_P2P_PriceList.xlsx'
  else
    scExcelExport.Filename := ExtractFilePath(Application.ExeName) + 'Excel\CarHire_P2P_PriceList_1_10.xlsx';
  ...
  GpSds.Close;
  GpSds.CommandText := 'exec [p_CarHire_P2P_PriceList_GST] ' +
        QuotedStr(FormatDateTime('mm/dd/yyyy',x_TravelDate)) + ', ' +
  ...
  if (x_ReportType = 1) then
   if (not GpSds.Eof) and (GpSds['TourOperatorGstPerc'] <> null) then
     scExcelExport.ExcelWorkSheet.Range['U3','U3'].Value := 'Tour Gst @ ' + FormatFloat('#,##0.0',GpSds['TourOperatorGstPerc']) + '%';
  ...
  while (not GpSds.EOF) do
    begin
      ...
      x_Sector := GpSds['Sector'];
      if (x_Sector <> x_PrevSector) then
        PrintSector (scExcelExport, GpSds, x_row);

      x_Agents_id := GpSds['Agents_id'];
      if (x_count = 0) then
        x_type := 2
      else if (x_Sector <> x_PrevSector) then
        x_type := 3
      ...
      x_PrevSector := x_Sector;
      x_PrevAgents_id := x_Agents_id;

      if (x_count = 0) and (GpSds['MarginPerc'] <> null) and (x_ReportType = 1) then
        scExcelExport.ExcelWorkSheet.Range['R6','R6'].Value := '@ ' + FormatFloat('#,##0',GpSds['MarginPerc']) + '%';
  ...
  scExcelExport.ExcelWorkSheet.Range['I'+IntToStr(x_StartRow),'U'+IntToStr(x_EndRow)].NumberFormat := '#,##0';
  SetColumnColours (scExcelExport, x_StartRow, x_EndRow, x_ReportType);

  if (x_margin = '0') then
    scExcelExport.ExcelWorkSheet.Range['R1','U1'].EntireColumn.Hidden := true;
...
begin
  ...
  if GpSds['Sector'] <> null then
    scExcelExport.ExcelWorkSheet.Range['B'+IntToStr(x_row),'B'+IntToStr(x_row)].Value := GpSds['Sector'];
...
begin
  ...
  x_cell := NextColumn(x_cell);
  if (GpSds['RepCost'] <> null) and (GpSds['RepCost'] <> 0) then
    scExcelExport.ExcelWorkSheet.Range[x_cell+IntToStr(x_row),x_cell+IntToStr(x_row)].Value := GpSds['RepCost'];
...
procedure GST_Car_P2P_LineWise_PriceList (scExcelExport: TScExcelExport; x_TravelDate: TDateTime; x_Currencies_id: integer; x_StateStr, x_Margin,
                                          x_FileName: string; x_ReportType, x_OptionOrder, x_OptionIndia: integer);
var
  GpSds: TSQLDataSet;
  x_row, x_StartRow, x_EndRow, x_type, x_count : integer;
...
begin
  ...
  if (x_ReportType = 4) then
    begin
      scExcelExport.Filename := ExtractFilePath(Application.ExeName) + 'Excel\CarHire_P2P_PriceList_1_10_SingleLine.xlsx'
    end;
...
begin
  ...
  Gp2Sds.CommandText := 'SELECT v.Vehicle, t.ToPax, t.FinalQuoteCurr ' +
   ...
   'AND t.FromCities_id = ' + IntToStr(GpSds['FromCities_id']) + ' ' +
   'AND t.ToCities_id = ' + IntToStr(GpSds['ToCities_id']) + ' ' +
