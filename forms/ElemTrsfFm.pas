...
type
  TElemTrsfForm = class(TCustom_1MG_Form)
    Label1: TLabel;
...
var
  ElemTrsfForm: TElemTrsfForm;
  ElemTrsfForm_Level: Integer;
  ElemTrsfForm_NullDate: TDateTime;
  ElemTrsfForm_Cities_id: integer;
  ElemTrsfForm_Agents_id: integer;
  ElemTrsfForm_Services_id: integer;
  ElemTrsfForm_Wef: TDateTime;
...
procedure TElemTrsfForm.FormCreate(Sender: TObject);
begin

  ElemTrsfForm_NullDate := StrToDate('01/01/1970');
  ...
  MasterSds.Params[1].Value := ElemTrsfForm_NullDate;
  inherited;
  ...
  if ElemTrsfForm_Cities_id <> null then
    cxCityLCMB.EditValue := ElemTrsfForm_Cities_id;

  if ElemTrsfForm_Agents_id <> null then
    cxAgentLCMB.EditValue := ElemTrsfForm_Agents_id;

  if ElemTrsfForm_Wef <> null then
    cxWefLCMB.EditValue := ElemTrsfForm_Wef;

  ServicesCds.Locate('Services_id', ElemTrsfForm_Services_id, []);

end;

procedure TElemTrsfForm.FormClose(Sender: TObject;
...
procedure TElemTrsfForm.FormDestroy(Sender: TObject);
begin
  inherited;
  ElemTrsfForm := nil;
end;

procedure TElemTrsfForm.ActivateInCustom;
begin
  Custom_MasterCdsName := 'ElemServices';
  Custom_MasterKeyField := 'ElemServices_id';
  Custom_AdmLevel := ElemTrsfForm_Level;
...
procedure TElemTrsfForm.FormActivate(Sender: TObject);
begin
  inherited;
  ActivateInCustom;
  cxGrid1.SetFocus;
...
procedure TElemTrsfForm.MasterCdsBeforePost(DataSet: TDataSet);
begin
...
function TElemTrsfForm.IsMasterRecordDuplicate: Boolean;
var
...
procedure TElemTrsfForm.FilterData;
var
...
procedure TElemTrsfForm.FilterWef;
var
  x_Agents_id: integer;
  x_wef: TDateTime;
begin
...
procedure TElemTrsfForm.cxCityLCMBPropertiesEditValueChanged(
  Sender: TObject);
var
  x_Cities_id: integer;
begin
...
procedure TElemTrsfForm.MasterCdsAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TElemTrsfForm.SpeedButton1Click(Sender: TObject);
var
  x_Wef: TDateTime;
begin
  inherited;
...
procedure TElemTrsfForm.cxAgentLCMBPropertiesEditValueChanged(
  Sender: TObject);
begin
  inherited;
  FilterWef;
