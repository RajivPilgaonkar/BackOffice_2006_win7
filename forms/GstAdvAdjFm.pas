...
type
  ...
  private
    ...
    procedure UpdateAdvanceGstAmount (x_advances_id: integer);
    procedure UpdateAdvanceAdjGstAmount (x_advancesAdj_id: integer);
  public
    { Public declarations }
  end;
...
begin
  ...
  Custom_DetailCdsName := 'AdvancesAdj';
  Custom_DetailKeyField := 'AdvancesAdj_id';
...
begin
  ...
  if (MasterCds['ExchRate'] <> 1.0) then
    begin
      ...
      x_tax := MasterCds['AdvAmt'].AsDouble - MasterCds['AdvAmt'].AsDouble/(1.0 + MasterCds['Rate'].AsDouble/100.0);
...
begin
  ...
  if (DetailCds['AdjDate'] < MasterCds['TransactionDate']) then
    raise Exception.Create('Adj Date should be later than the Advance Date');
  ...
  if (MasterCds['ExchRate'] <> 1.0) then
    begin
      x_tax := DetailCds['AdjAmt'].AsDouble - DetailCds['AdjAmt'].AsDouble/(1.0 + MasterCds['Rate'].AsDouble/100.0);
      DetailCds['I_GST'] := x_tax;
      DetailCds['C_GST'] := 0.0;
      DetailCds['S_GST'] := 0.0;
    end
  else if (MasterCds['PlaceOfSupply'] <> '30-Goa') then
    ...
    begin
      x_tax := DetailCds['AdjAmt'].AsDouble - DetailCds['AdjAmt'].AsDouble/(1.0 + MasterCds['Rate'].AsDouble/100.0);
      DetailCds['I_GST'] := 0.0;
      DetailCds['C_GST'] := x_tax/2.0;
      DetailCds['S_GST'] := x_tax/2.0;
    end;
...
begin
  ...
  x_Result := false;
  if x_amt > MasterCds['AdvAmt'] then
    x_Result := true;
...
procedure TGstAdvAdjForm.MasterCdsAfterPost(DataSet: TDataSet);
var
  x_advances_id: integer;
begin
  inherited;

  x_advances_id := MasterCds['Advances_id'];
  UpdateAdvanceGstAmount (MasterCds['Advances_id']);

  MasterCds.Locate('Advances_id',x_advances_id,[]);

end;

procedure TGstAdvAdjForm.UpdateAdvanceGstAmount (x_advances_id: integer);
var
  GpSds : TSQLDataSet;
  QueryString: string;
begin

  QueryString := 'EXEC p_UpdateAdvanceAmount ' + IntToStr(x_advances_id) +  ' ';
...
procedure TGstAdvAdjForm.DetailCdsAfterPost(DataSet: TDataSet);
begin
  inherited;

  UpdateAdvanceAdjGstAmount (DetailCds['AdvancesAdj_id']);
...
procedure TGstAdvAdjForm.UpdateAdvanceAdjGstAmount (x_advancesAdj_id: integer);
var
  GpSds : TSQLDataSet;
  QueryString: string;
begin

  QueryString := 'EXEC p_UpdateAdvanceAdjAmount ' + IntToStr(x_advancesAdj_id) +  ' ';
