...
type
  TMiscPriceListForm = class(TForm)
...
var
  MiscPriceListForm: TMiscPriceListForm;
...
procedure TMiscPriceListForm.FormCreate(Sender: TObject);
begin
  Height := 600;
  Width := 680;
...
function TMiscPriceListForm.GetDefaultMargin: double;
var
  GpSds: TSQLDataSet;
  x_Margin: double;
begin
...
procedure TMiscPriceListForm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TMiscPriceListForm.FormDestroy(Sender: TObject);
begin
  MiscPriceListForm := nil;
end;

procedure TMiscPriceListForm.Statewise1Click(Sender: TObject);
begin
...
procedure TMiscPriceListForm.Citywise1Click(Sender: TObject);
begin
...
procedure TMiscPriceListForm.PrintReport(x_option: integer; x_sightseeing: integer);
var
...
begin
  ...
  GpSds.Close;
  GpSds.CommandText := ' EXEC p_SightSeeingOtherPriceList ''' + FormatDateTime('mm/dd/yyyy',cxFromDateEdit.Date) + ''', ''' +
...
procedure TMiscPriceListForm.PrintReportToExcel (GpSds: TSQLDataSet);
var
...
begin
  ...
  with scExcelExport.ExcelWorkSheet do
    begin
      ...
      Range['A1','A1'].Value := 'Misc Cost as of ' + FormatDateTime('dd/mm/yyyy',cxFromDateEdit.Date);
...
//      Range['A3','A3'].Value := 'Cost based on ' + trim(cxNumPaxEdit.Text) + ' pax';
      ...
      Range['E3','S4'].Font.FontStyle := 'Bold';
      Range['E3','S4'].Font.Size := '10';
      ...
      if cxMarginCkb.Checked and cxDetailsCkb.Checked then
        begin
          ...
          Range['R3','R3'].Value := 'Tour Serv Tax';
...
//  scExcelExport.ExcelWorkSheet.Range['AN5','AP5'].Merge(3);
  ...
  x_FileName := g_ReportDir + '\MiscRates.xls';
...
function TMiscPriceListForm.GetCurrencyCode(x_Currencies_id: integer): string;
var
...
procedure TMiscPriceListForm.PrintNextState(x_State: string; var x_row: integer);
begin
...
procedure TMiscPriceListForm.PrintNextCity(x_City: string; var x_row: integer);
begin
...
procedure TMiscPriceListForm.PrintNextService(x_Service: string; var x_row: integer; x_option: integer);
begin
...
procedure TMiscPriceListForm.PrintRecord(GpSds: TSQLDataSet; var x_row: integer; x_option: integer);
begin
...

{
      ...
      if (GpSds['Type'] <> null) and (GpSds['Type'] = 1) then
        Range['K'+IntToStr(x_row),'K'+IntToStr(x_row)].Value := 'Per Person'
      else if (GpSds['Type'] <> null) and (GpSds['Type'] = 2) then
        Range['K'+IntToStr(x_row),'K'+IntToStr(x_row)].Value := 'Per Group';
      ...
      if cxDetailsCkb.Checked then
        begin
          ...
          if (GpSds['MiscCost'] <> null)  then
            Range['L'+IntToStr(x_row),'L'+IntToStr(x_row)].Value := GpSds['MiscCost'];
...
procedure TMiscPriceListForm.CloseBtnClick(Sender: TObject);
begin
  Close;
end;
